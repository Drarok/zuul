...
class SyncCommand extends BaseCommand {
  execute(args) {
    ...
    for (let server of servers.getAll()) {
      ...
      this.syncKeys(server, options, unique);
    }
  }

  syncKeys(server, options, keys) {
      ...
      conn.sftp((err, sftp) => {
        ...
        sftp.realpath('.ssh/authorized_keys', (err, absPath) => {
          ...
          stream.on('data', buffer => {
            // TODO: Parse existing keys
            console.log(`${server.name} read stream data`.yellow);
          });

          stream.on('end', () => {
            console.log(`${server.name} read stream end`.yellow);
            stream.close();
            ...
            stream.write(keys.join('\n'), () => {
              console.log(`${server.name} write stream data`.yellow);
              stream.close();
              conn.end();
            });
          });
