...
class EnrollCommand extends BaseCommand {
  execute(args) {
    ...
    if (args.password) {
      this.connect(hostname, args.password, args.verbose);
      return;
    }
  ...
  usage() {
    console.log('zuul enroll <name> <user@hostname>');
  }
  ...
  connect(hostname, password, verbose) {
    ...
    this.getConnection(hostname, password)
      ...
      .then(keys => {
        ...
        // TODO: Import existing keys
        // TODO: Add this server to zuul on success

        return this.writeKeys(sftp, keys);
      })
      .then(() => {
        conn.end();
        console.log(`Successfully enrolled ${hostname}`.green);
      })
      .catch(err => {
        if (conn) {
          conn.end();
