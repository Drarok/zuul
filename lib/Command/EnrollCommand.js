...
class EnrollCommand extends BaseCommand {
  execute(args) {
    ...
    if (serverRepo.find(identifier)) {
      throw new Error('Server ${identifier} already exists');
    }

    const server = new Server(identifier);

    const groupRepo = this.app.get('groups');
    const keyRepo = this.app.get('keys');

    if (args.group) {
      const groups = Array.isArray(args.group) ? args.group : [args.group];
      for (const name of groups) {
        const group = groupRepo.find(name);
  ...
  usage() {
    console.log('zuul enroll <user@hostname> [...] [--group=<group name>] [--key=<key name>]');
  }
